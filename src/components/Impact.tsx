        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          {metrics.map((m) => (
            <div key={m.label} className="text-center lg:text-left">
              <p className="text-[2.5rem] lg:text-[3rem] font-bold text-foreground tracking-tight leading-none">{m.value}</p>
              <p className="text-foreground/50 text-[13px] mt-2 uppercase tracking-wider">{m.label}</p>
            </div>
          ))}
        </div>
      </div>
